...
export class UseMcpToolHandler implements IFullyManagedTool {
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		if (mcp_arguments) {
			try {
				...
				await config.callbacks.say(
					"error",
					`NeoCode Agentic tried to use ${tool_name} with an invalid JSON argument. Retrying...`,
				)
		...
		if (config.callbacks.shouldAutoApproveTool(block.name) && isToolAutoApproved) {
			...
			const notificationMessage = `NeoCode wants  to use ${tool_name || "unknown tool"} on ${server_name || "unknown server"}`
